...
case $1 in
    calc)
        ...
        if [[ "$2" = "div" && "$4" == "-0" ]]; then
            echo "Ошибка: на такой ноль тоже делить нельзя. +0 даже не проверяй." >&2; exit -5
        fi
        if [[ "$2" = "div" && "$4" == "+0" ]]; then
            echo "Ошибка: на такой ноль делить нельзя так же, как и на любой другой." >&2; exit -5
        fi
    ...
    "exit")
        ...
        if  [ -z "$2" ]; then
           ...
           if ! [[ $2 =~ $reg ]]; then
               echo "Ошибка: строка не может быть кодом ошибка." >&2; exit -5
           fi
           exit_ "$2"
        fi
        ;;
