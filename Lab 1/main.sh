...
case $1 in
    ...
    strlen)
        if ! [ -f "strlen.sh" ]; then
            echo "Ошибка: стрлен спиздили." >&2; exit -5
        fi
        . ./strlen.sh
        if ! [[ $# -eq 2 ]]; then
            echo "Ошибка: для корректной работы стрлена нужно указать верное количество параметров (1). Спасибо за внимание." >&2; exit -1
        fi
    ...
    log)
        if ! [ -f "log.sh" ]; then
            echo "Ошибка: лог спиздили." >&2; exit -5
        fi
        . ./log.sh
        if ! [ -f "/var/log/anaconda/X.log" ]; then
            echo "Ошибка: файла X.log не сущесвует в искомой директории. Спасибо за внимание." >&2; exit -3
        fi
    ...
    exit)
        if ! [ -f "exit.sh" ]; then
            echo "Ошибка: экзит спиздили." >&2; exit -5
        fi
        if ! [[ $# -eq 1 ]]; then
            echo "Ошибка: для корректной работы экзита нужно указать верное количество параметров (1). Спасибо за внимание." >&2; exit -1
        fi
